...
void add_course(Student students[], int student_count, int student_id, int course_id, int is_core, int *nfc){
    for (int i=0; i<student_count; i++){
        if (students[i].student_id==student_id){
            if (is_core){
                if ((course_id>=201 && course_id<=204) || (course_id>=401 && course_id<=404) || (course_id>=601 && course_id<=604)){
                    for (int j=0; j<MAX_CORE; j++){
                        if (students[i].core[j]==0){
                ...
                if (course_id==205 || course_id==206 || course_id==405 || course_id==406 || course_id==605 || course_id==606){
                    for (int j=0; j<MAX_ELECTIVE; j++){
                        if (students[i].elective[j]==0){
...
int add_course_student(int student_id, int sem){
    ...
    while (1){
        ...
        scanf("%d", &course_id);
        int not_found_course=0;
        add_course(students, student_count, student_id, course_id, is_core, &not_found_course);
        if (not_found_course)
...
int find_student_course(int student_id, int course_id) {
    ...
    int file_student_id, file_course_id, file_status;
    while (fscanf(file, "%d,%d,%d\n", &file_student_id, &file_course_id, &file_status) == 3) {
        if (file_student_id == student_id && file_course_id == course_id) {
            fclose(file);
            return 1 ;
        }
    }
...
void save(int student_id, int course_id, int status, int fid) {
    ...
    fprintf(file, "%d,%d,%d,%d\n", student_id, course_id, status, fid);
    fclose(file);
}

//check if faculty has already been alloted.
int check_allotment_status(int student_id, int course_id, const char *file_name) {
    ...
    while (fgets(line, sizeof(line), file)) {
        int field_count = parse_line(line, fields);
        if (field_count >= 2) { // Assuming at least two fields are present (student_id and course_id)
            int file_student_id = atoi(fields[0]); // Assuming student_id is the first field
            int file_course_id = atoi(fields[1]); // Assuming course_id is the second field
...
void update_faculty(const char *filename, const char *faculty_id, const char *course_id, const char *new_seats) {
    ...
    while (fgets(line, sizeof(line), file) != NULL) {
        ...
        if (field_count > 2 && strcmp(fields[0], faculty_id) == 0 && strcmp(fields[1], course_id) == 0) {
            snprintf(fields[4], MAX_LINE_LENGTH, "%s", new_seats);
            updated = 1;
    ...
    if (updated) {
        ...
        printf("No matching faculty and course found. No update made.\n");
        remove(temp_filename);
    }
}

//Function to handle selection of faculty
void handle_faculty_selection(int course_id,int sid){
    char faculty_id[10];
    char new_seats[5];
    int seats;
    char cid[10];
            ...
            for (int j = 0; j < students[i].num_core; j++) {
                if (students[i].core[j] == course_id) {
                    ...
                    if (m != 1) {
                        ...
                        printf("Faculty has already been allotted for core course %d\n", course_id);
                    }
                    break;
                }
            }
            ...
            for (int j = 0; j < students[i].num_elective; j++) {
                if (students[i].elective[j] == course_id) {
                    ...
                    if (m != 1) {
                        ...
                        printf("Faculty has already been allotted for elective course %d\n", course_id);
                    }
                    break;
                }
            }
